...
function doElementLevelChecks(element_, isTechniquePageWide_) {
	...
	if(/[^\s]\s+[^\s]/.test(element_.textContent)) {
		throw new SsmlError(`This element's text content has multiple words (separated by whitespace).  This add-on doesn't support this.  This is because NVDA, when the user is navigating with the up/down arrow keys, will obviously break up sentences into "chunks" or "lines".  (This kind of "line" usually doesn't correspond to a visual line.  "Line" here means whatever NVDA's up/down arrow keys move across.)  NVDA will often, more-or-less at random, consider the second word in any pair of words to be part of the next "line".  In order for this add-on to work, each unit of "plain text" (i.e. the text that this add-on will override the spoken presentation of) needs to be sent to the speech synth in one function call.  That means that the entire plain text needs to be contained in one "line".  If the plain text has multiple words, then there is no guarantee of this.  So this add-on refuses to try to handle it.`);
	}
